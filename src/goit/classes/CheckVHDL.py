...
class CheckVHDL(Check):
    ...
    def __init__(self, argv):
        ...
        self.elements = {'library'     : {'name': 'library',      'enabled': True, 'parent': [],                                       'regex': r'(?i)\s*(library\s+\S+\s*;)'},
                        'use'          : {'name': 'use',          'enabled': True, 'parent': [],                                       'regex': r'(?i)\s*(use\s+\S+[.]\S+\s*;)'},
                        'entity'       : {'name': 'entity',       'enabled': True, 'parent': [],                                       'regex': r'(?im)^\s*(entity\s+(?P<id>\S+)\s+is[\S\s]+?end(\s+entity)?(\s+(?P=id))?\s*;)'},
                        'architecture' : {'name': 'architecture', 'enabled': True, 'parent': [],                                       'regex': r'(?im)^\s*(architecture\s+(?P<id>\S+)\s+of\s+\S+\s+is[\S\s]+?begin[\S\s]+?end(\s+architecture)?(\s+(?P=id))?\s*;)'},
                        'generic'      : {'name': 'generic',      'enabled': True, 'parent': ['component', 'entity', 'configuration'], 'regex': 'parentheses'},
                        'port'         : {'name': 'port',         'enabled': True, 'parent': ['component', 'entity', 'configuration'], 'regex': 'parentheses'},
                        'port map'     : {'name': 'port map',     'enabled': True, 'parent': ['instance'],                             'regex': 'parentheses'},
                        'generic map'  : {'name': 'generic map',  'enabled': True, 'parent': ['instance'],                             'regex': 'parentheses'},
                        'generate'     : {'name': 'generate',     'enabled': True, 'parent': ['architecture'],                         'regex': r'(?im)^\s*((?P<id>\S+)\s*:\s*(for\s+[\S\s]+?\sin\s+[\S\s]+?\s|if\s+[\S\s]+?\s+)generate\s+[\S\s]+?end\s+generate(\s+(?P=id))?\s*;)'},
                        'instance'     : {'name': 'instance',     'enabled': True, 'parent': ['architecture'],                         'regex': r'(?im)begin[\S\s]*?^\s*(\w+\s*:(?!\s*if\s+)(\s*component|\s*entity|\s*configuration)?\s+[\S\s]*?;)'}}
